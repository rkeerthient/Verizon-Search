...
const ContactInfoCard = (props: CardProps<Ce_contactInformation>): JSX.Element => {
  const { result } = props;
  const { name } = result;
  ...
  return (
    <div className="w-full my-4 border p-4 bg-white flex justify-between pr-8">
      ...
      {richTextDescriptionV2 && <div className="w-3/4">
      ...
      <div className=" mr-9 my-auto">
          <a className="font-bold hover:underline" href={landingPageUrl}>Learn more {`>`}</a>
        </div>
    </div>
  );
};
